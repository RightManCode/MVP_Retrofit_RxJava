...
import com.wzgiceman.rxretrofitlibrary.retrofit_rx.downlaod.dao.ThreadInfoManager;
...
public class DownloadTask {
    ...
    /**线程数据库操作**/
    private ThreadInfoManager mThreadDao;
    /**停止下载标志位**/
    ...
    public DownloadTask(Context mContext, TaskInfo taskInfo, ExecutorService threadPool, int threadCount, INoticeListener noticeListener) {
...
//        mDownloadList = new LinkedList<DownloadThread>();
        mThreadDao = ThreadInfoManager.getInstance();
        if(threadCount > 0) {
            ...
            mHandler = new Handler() {
                ...
                public void dispatchMessage(Message msg) {
                    ...
                    if(msg.what == MSG_DOWNLOADING) {
                        ...
                        // 更新本地文件信息
                        mTaskInfo.setFinished(progress);

                        // 发送广播 让通知栏进行进度的提示
                        if (mNoticeListener != null){
                            if (progress>100) progress = 100;
                            mNoticeListener.noticeBrief(mTaskInfo.getFileName(), progress);
                        }

                        if (progress>=100){
                            mTaskInfo.setState(DownState.FINISH);
                        }
                        TaskInfoManager.getInstance().update(mTaskInfo);
        ...
        if(threadInfoList.size() <=0) {
            ...
            if(block > 0) {
                ...
                for(int i = 0;i < mThreadCount;i++) {
                    ThreadInfo info = new ThreadInfo(Tools.getId(),i,mTaskInfo.getUrl(),i*block,(i+1)*block-1,0);
                    if(i == mThreadCount -1) {
                        info.setEnd(mTaskInfo.getLenght()); //分段最后一个，结束位置到文件总长度末尾
                    }
                    threadInfoList.add(info);         //加入列表
                    mThreadDao.insert(info);   //向数据库插入线程信息
                }
                //end 根据线程数量分别建立线程信息
            }else {
                ThreadInfo info = new ThreadInfo(Tools.getId(),0,mTaskInfo.getUrl(),0,mTaskInfo.getLenght(),0);
                threadInfoList.add(info);
                mThreadDao.insert(info);
            }
        }
...
//            intent.putExtra("fileInfo", mTaskInfo) ;
    ...
    private void sendBroadcastDownloadSuc() {
        ...
        intent.putExtra(DownloadService.EXTRA_FILE_NAME,mTaskInfo.getFileName());  //下载的文件名
        intent.putExtra(DownloadService.EXTRA_DOWNLOAD_PROGRESS, String.valueOf(mTaskInfo.getFinished()));
      /*  intent.putExtra(DownloadService.EXTRA_FILE_NAME,mTaskInfo.getFileName());
        intent.putExtra(DownloadService.EXTRA_FILE_PATH,mTaskInfo.getFilePath());*/
        mContext.sendBroadcast(intent);
    }
     ...
     * 广播通知，通知栏的更新
     */
    /*private void sendBroadcastDownloadBrief() {
        Intent intent = new Intent();
        intent.setAction(DownloadService.ACTION_NOTICE_BRIEF);
        intent.putExtra(DownloadService.EXTRA_FILE_NAME,mTaskInfo.getFileName());  //下载的文件名
        intent.putExtra(DownloadService.EXTRA_DOWNLOAD_PROGRESS, String.valueOf(mTaskInfo.getFinished()));
        mContext.sendBroadcast(intent);
    }*/
...
//                                Log.i(tag, "readLen = " + readLen);
                            notifyProgress(threadInfo.getThreadId(), finished); //每隔2秒通知下载进度
                            time = System.currentTimeMillis();
                        }
                        //start 停止下载，保存进度
                        if(isPause) {
                            Log.i(tag,"pause name = "+mTaskInfo.getFileName());
                            notifyProgress(threadInfo.getThreadId(), finished);        //通知下载进度
                            mThreadDao.updateThread(threadInfo.getUrl(),threadInfo.getThreadId(),finished);  //更新数据库对应的线程信息
                            return;
                        }
                        //end 停止下载，保存进度
                    }
...
//                        broadcastFinished(threadInfo.getId(),finished);
                    notifyProgress(threadInfo.getThreadId(),finished);
                }
